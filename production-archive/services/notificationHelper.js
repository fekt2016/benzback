...
const createAdminNotification = async (notificationData) => {
  try {
    ...
    // WASM MEMORY OPTIMIZATION: Process in batches of 10 to prevent memory spikes
    // If there are 100+ admins, Promise.all() would create 100+ concurrent DB operations
    // Batch processing limits concurrent operations and reduces memory usage
    ...
    // WASM MEMORY OPTIMIZATION: Clean up after batch operations
    if (global.gc && adminUsers.length > 20) {
      // Trigger GC after large batch operations to free WASM memory
      global.gc();
      console.log("🧹 GC triggered after batch notification creation");
    }
