...
========================= */
// WASM MEMORY OPTIMIZATION: Strict payload limits to prevent large in-memory allocations
// Large JSON payloads can cause Undici (WebAssembly) to allocate excessive memory
// 200kb is sufficient for most API requests and prevents WASM memory exhaustion
app.use(express.json({ limit: "200kb" })); // Reduced to 200kb to prevent WASM memory issues
app.use(express.urlencoded({ extended: true, limit: "200kb" })); // Reduced to 200kb
